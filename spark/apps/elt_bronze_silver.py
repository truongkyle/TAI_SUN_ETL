...
from utils.minio_client import get_spark_session
# --- CẤU HÌNH ---
...
def process_bronze_to_silver(spark, table_name,unique_keys = "col"):
    """
    Logix xử lý chính: Read Bronze -> Transform -> Merge Silver
    """
    bronze_path = f"{BRONZE_BUCKET}/{table_name.lower()}"
    silver_path = f"{SILVER_BUCKET}/{table_name.lower()}"
...
if __name__ == "__main__":
    spark = get_spark_session(app_name="ELT_BronzeToSilver")
    ...
    for tbl in tables:
        process_bronze_to_silver(spark, tbl)
